...
// Generated at 2018-06-22 22:36:23 (7057ms)
globalvar gml_std_haxe_type_is;gml_std_haxe_type_is=ds_grid_create(221,221);
for(var _g_type=0;_g_type<221;_g_type+=1)gml_std_haxe_type_is[#_g_type, _g_type]=true;
...
globalvar mt_gml_std_haxe_class;mt_gml_std_haxe_class=gml_std_haxe_class_create(64,"gml_std_haxe_class");
globalvar mt_gml_func_scope;mt_gml_func_scope=gml_std_haxe_class_create(208,"gml_func_scope");
globalvar mt_gml_with_scope;mt_gml_with_scope=gml_std_haxe_class_create(220,"gml_with_scope");
...
/// array_hx_push(arr:array<T>, val:T)->int
...
/// array_hx_slice(arr:array<T>, pos:int, ?end:int)->array<T>
...
/// array_hx_copy(arr:array<T>)->array<T>
...
if (live_enabled) {
	...
	while(l__g<array_length_1d(l_sources)){
		...
		if(l_b[8]==undefined){
			...
			if(this[10]!=undefined)this[@10]+=chr(10)+l_errorNext; else this[@10]=l_errorNext;
		} else {
			this[@10]=l_b[8];
			this[@11]=l_b[9];
			return this;
	...
	if(gml_program_check(this)){
		...
		if(this[10]!=undefined)this[@10]+=chr(10)+g_gml_compile_error_text; else this[@10]=g_gml_compile_error_text;
...
if (live_enabled) {
	...
	while(l__g1<l__g){
		...
		l_r+="#define "+l_scr[0]+chr(10)+gml_action_list_print(l_scr[9]);
	}
	return l_r;
} else return undefined;
...
#define is_type
/// is_type(value:dynamic, type:Class<T>)->bool
...
if (live_enabled) {
	...
	this[@6]=gml_pos_create(this,string_count(chr(10),l_code)+2,1);
}
return this;
...
#define gml_std_haxe_type_proto
/// gml_std_haxe_type_proto(...rest:any)->any
if (live_enabled) {
	var l_n=argument_count;
	var l_out=array_create(l_n,0);
	for(var l_i=0;l_i<l_n;l_i++){
		l_out[@l_i]=argument[l_i];
	}
	return l_out;
} else return undefined;
...
/// gml_seek_eval_node_to_value(node:GmlNode)->GmlValue
...
/// gml_seek_eval_value_to_node(val:GmlValue, d:GmlNodeData)->GmlNode
...
if (live_enabled) {
	...
	var l_n=ds_list_size(l_qbs);
	var l_i=-1;
	while(++l_i<l_n){
		var l_qb=l_qbs[|l_i];
		var l_rb=l_i;
		var l_rbi=asset_get_index(l_qb[?"background"]);
		background_visible[l_rb]=l_qb[?"visible"];
		background_index[l_rb]=l_rbi;
		background_foreground[l_rb]=l_qb[?"foreground"];
		background_x[l_rb]=l_qb[?"x"];
		background_y[l_rb]=l_qb[?"y"];
		background_hspeed[l_rb]=l_qb[?"hspeed"];
		background_vspeed[l_rb]=l_qb[?"vspeed"];
		background_htiled[l_rb]=l_qb[?"htiled"];
		background_vtiled[l_rb]=l_qb[?"vtiled"];
		if(l_qb[?"stretch"]&&l_rbi!=-1){
	...
	l_n=ds_list_size(l_qvs);
	l_i=-1;
	while(++l_i<l_n){
		var l_qv=l_qvs[|l_i];
		var l_rv=l_i;
		view_visible[l_rv]=l_qv[?"visible"];
		view_xview[l_rv]=l_qv[?"xview"];
		view_yview[l_rv]=l_qv[?"yview"];
		view_wview[l_rv]=l_qv[?"wview"];
		view_hview[l_rv]=l_qv[?"hview"];
		view_xport[l_rv]=l_qv[?"xport"];
		view_yport[l_rv]=l_qv[?"yport"];
		view_wport[l_rv]=l_qv[?"wport"];
		view_hport[l_rv]=l_qv[?"hport"];
		view_hspeed[l_rv]=l_qv[?"hspeed"];
		view_vspeed[l_rv]=l_qv[?"vspeed"];
		view_hborder[l_rv]=l_qv[?"hborder"];
		view_vborder[l_rv]=l_qv[?"vborder"];
		view_object[l_rv]=asset_get_index(l_qv[?"object"]);
	}
	...
	while(++l_i<l_n){
		...
		var l_rtile=tile_add(l_id,l_qtile[?"l"],l_qtile[?"t"],l_qtile[?"w"],l_qtile[?"h"],l_qtile[?"x"],l_qtile[?"y"],l_qtile[?"z"]);
		l_f=l_qtile[?"c"];
		if(l_f!=4294967295.){
			var l_h=l_f;
	...
	while(++l_i<l_n){
		...
		var l_rnext=instance_create(l_qinst[?"x"],l_qinst[?"y"],l_base);
		with (l_rnext) {
			var l_rname=l_qinst[?"s"];
			gml_const_add(l_rname,self);
			l_f=l_qinst[?"r"];
			if(l_f!=0)self.image_angle=l_f;
			l_f=l_qinst[?"sx"];
			if(l_f!=1)self.image_xscale=l_f;
			l_f=l_qinst[?"sy"];
			if(l_f!=1)self.image_yscale=l_f;
			l_f=l_qinst[?"c"];
			if(l_f!=4294967295.){
				var l_h1=l_f;
			...
			l_s=l_qinst[?"cc"];
			if(l_s!="")live_room_loader_run_cc(l_s,l_rname+":CreationCode");
		}
	}
...
/// gml_object_field(object:object, field:string, func:function[ :bool :GmlValue]:GmlValue)
...
/// gml_func_scope_create(actions:GmlActionList, offset:int, args:GmlValueList, locals:GmlValueList, self:GmlValue, other:GmlValue, next:func_scope)
...
/// gml_thread_create(program:program, actions:GmlActionList, args:GmlValueList, locals:GmlValueList, ?self:GmlValue, ?other:GmlValue, offset:int = 0)
...
/// vm_gml_thread_exec_call_fn1(f:dynamic, w:array<dynamic>, n:int)->null<GmlValue>
...
/// vm_gml_thread_exec_call_fn2(f:dynamic, w:array<dynamic>, n:int)->null<GmlValue>
...
/// vm_gml_thread_exec_call_fn3(f:dynamic, w:array<dynamic>, n:int)->null<GmlValue>
...
/// vm_gml_thread_exec_call_fn4(f:dynamic, w:array<dynamic>, n:int)->null<GmlValue>
...
/// vm_gml_thread_exec_call_fn5(f:dynamic, w:array<dynamic>, n:int)->null<GmlValue>
...
/// vm_gml_thread_exec_call_fn6(f:dynamic, w:array<dynamic>, n:int)->null<GmlValue>
...
/// vm_gml_thread_exec_call_fn7(f:dynamic, w:array<dynamic>, n:int)->null<GmlValue>
...
/// vm_gml_thread_exec_call_fn8(f:dynamic, w:array<dynamic>, n:int)->null<GmlValue>
...
/// vm_gml_thread_exec_call_fn9(f:dynamic, w:array<dynamic>, n:int)->null<GmlValue>
...
/// vm_gml_thread_exec_call(fn:dynamic, args:array<dynamic>, argc:int)->GmlValue
...
/// gml_value_list_copy(this:array<GmlValue>)->GmlValueList
var l_this1=argument[0];
if (live_enabled) {
